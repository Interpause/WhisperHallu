...
useDemucs = True
if useDemucs:
    from demucsWrapper import demucs_audio, load_demucs_model
    ...
    try:
        if whisperFound == "FSTR":
            ...
            model = whisper.load_model(
                modelSize, device=torch.device("cuda:" + gpu)
            )  # May be "cpu"
        print("LOADED")
    except:
    ...
    with open(aLog) as f:
        ...
        for line in lines:
            if re.match(
                ...
                duration = re.sub(
                    r"(^ *Duration: *|[,.].*$)", "", line, 2, re.IGNORECASE
                )
                return sum(
...
def transcribePrompt(
    path: str, lng: str, prompt=None, lngInput=None, isMusic=False, addSRT=False
):
    """Whisper transcribe."""
...
def transcribeOpts(path: str, opts: dict, lngInput=None, isMusic=False, addSRT=False):
    ...
    if useSpleeter:
        ...
        try:
            ...
            pathSpleeter = (
                spleeterDir
                + "/"
                + os.path.splitext(os.path.basename(pathIn))[0]
                + "/vocals.wav"
            )
    ...
    if useDemucs:
        ...
        pathDemucs = (
            pathIn + ".vocals.wav"
        )  # demucsDir+"/htdemucs/"+os.path.splitext(os.path.basename(pathIn))[0]+"/vocals.wav"
        ...
        demucs_audio(
            ...
            model=modelDemucs,
            device="cuda:" + cudaIdx,
            pathVocals=pathDemucs,
            pathOther=pathIn + ".other.wav",
        )
        print("T=", (time.time() - startTime))
    ...
    if addSRT:
        ...
        if isMusic:
            resultSRT = transcribeMARK(
                pathClean, opts, mode=3, lngInput=lngInput, isMusic=isMusic
            )
        else:
            resultSRT = transcribeMARK(
                pathNoCut, opts, mode=3, lngInput=lngInput, isMusic=isMusic
            )
...
def transcribeMARK(
    path: str, opts: dict, mode=1, lngInput=None, aLast=None, isMusic=False
):
    pathIn = path
        ...
        try:
            if mode != 3:
                ...
                aCmd = (
                    ...
                    + '" -i '
                    + mark2
                    + ' -filter_complex "[0:a][1:a][2:a]concat=n=3:v=0:a=1[a]" -map "[a]" -c:a pcm_s16le -ar '
                    + str(SAMPLING_RATE)
                    + ' "'
                    + pathMRK
                    + '" > "'
    ...
    try:
        ...
        if whisperFound == "FSTR":
            ...
            if mode == 3:
                ...
                output_path = os.path.join(p.parent, audio_basename + ".srt")
    ...
    aWhisper = (
        "(Whisper|Wisper|Wyspę|Wysper|Wispa|Уіспер|Ου ίσπερ|위스퍼드|ウィスパー|विस्पर|विसपर)"
    )
